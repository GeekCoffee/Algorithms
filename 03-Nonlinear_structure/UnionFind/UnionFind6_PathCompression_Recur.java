...
/**
 * 数组实现的并查集-版本6的UnionFind， date = 29/5 2019 ,  author = chensheng
 *  是在版本5-UnionFind的基础上，改变了Find时候的操作
 *  => 改变点：在Find方法中，递归寻找每个子节点的最高Root点，让每个子节点直接指向最高Root节点，整个树高就是2了
 *  => 性能消耗点：在系统栈上消耗过多，其时间性能上不一定比第5版的好
 *  ADT-基本：getSize
...
public class UnionFind6_PathCompression_Recur implements UnionFind{

    private int[] parent;  //i节点,parent[i]的值是i节点的父亲节点
    private int[] rank;  //表示以节点i为根节点的树的层数表示为rank


    public UnionFind6_PathCompression_Recur(int size){
        parent = new int[size];
        rank = new int[size];
