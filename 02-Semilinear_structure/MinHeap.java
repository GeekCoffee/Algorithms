...
/**
 *  以完全二叉树表示的一维数组实现的最小二叉堆,  date = 16/5 2019 ,  author = chensheng
 *  最小二叉堆的定义：任何局部的子二叉树(3个节点)都满足根节点最小，左右孩子节点都比根节点大
 ...
 *
 */
public class MinHeap<E extends Comparable<E>> { //继承Comparable类，让E类型的对象增加可比较性

    //引入自制的动态数组,来表示完全二叉树
    private ArrayList<E> data;

    public MinHeap(int capacity){ //开辟空间，但是还没有使用
    ...
    private void shiftUp(int i){
        while( i > 0 && data.get(parent(i)).compareTo(data.get(i)) > 0 ){ //当父节点大于当前操作节点时，循环继续
    ...
    public E extractMin(){
        ...
        //真正在完全二叉树中删除最小值节点，即在一维数组中删除最小值节点并保持二叉堆性质
        data.swap(0, data.getSize() - 1); //把最后一个节点与最小值接交换位置
        data.removeLast();  //在数组中删除换位后的最小值元素，维护好size
    ...
    private void shiftDown(int k){
        ...
        while(leftChild(k) < data.getSize()){
            int lesser = leftChild(k); //首先假设左孩子比右孩子小, lesser指向左孩子

            //当k的右孩子rigthChild的索引小于size的时候，说明k有右孩子，自己画图比较一下就知道了
            if(rightChild(k) < data.getSize() &&
                    data.get(leftChild(k)).compareTo(data.get(rightChild(k))) > 0)  //当k有右孩子且右孩子比左孩子小，与把lesser指向右孩子
                lesser = rightChild(k);

            //当父节点的值比孩子节点的值更小或者等于时，已经保持二叉堆的特性
            if(data.get(k).compareTo(data.get(lesser)) <= 0)
                break;
            data.swap(k, lesser); //否则，父节点k与自己孩子节点交换值
            k = lesser; //把k指向交换的孩子节点biggish索引指向的孩子节点上
        }
    }
...
//    }
    ...
    public static void main(String[] args){
        ...
        for(int i = 0; i < n; i++)
            minHeap.add(random.nextInt(200));   //几乎无序的随机整数，使用4Byte的MAX_VALUES


        //简单二叉堆排序，每次都从堆里取出最小的元素，顺序排列
        int[] arr = new int[n];
